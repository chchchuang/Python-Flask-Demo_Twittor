...
class User(UserMixin, db.Model): #大寫User是class, 默認小寫user是實例(db的表) # User繼承 db: 類似執行 MySQL上的 create table user
    ...
    FBuserID = db.Column(db.String(64), unique = True, index = True, nullable = True)
    FBaccessToken = db.Column(db.String(256), nullable = True)
    FBpicture = db.Column(db.String(128), nullable = True)
